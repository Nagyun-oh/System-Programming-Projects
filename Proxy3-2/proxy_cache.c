...
//  - process 내에서 스레드를 호출하여, 로그파일 기록을 수행           						               //
///////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
// Function: log_thread_func
// Input:
//   - arg : (void*) 형식으로 전달된 log_args 구조체 포인터
// Output:
//   - 없음 (파일에 로그 기록, 표준 출력에 TID 종료 메시지 출력)
// Purpose:
//   - URL 요청 결과(Hit/Miss)에 따라 로그 파일에 기록
//   - 쓰레드 ID와 함께 종료 메시지 출력
///////////////////////////////////////////////////////////////////////////////
void* log_thread_func(void* arg)
{
    struct log_args* args = (struct log_args*)arg; // 전달받은 매개변수 통해 초기화

    pthread_t tid = pthread_self(); // 현재 스레드 아이디
    ...
    //파일이 존재하지 않을시 스레드 종료 및 구조체 포인터 동적 할당 해제
    if(!file)
    {
        perror("fopen failed in thread");
        free(args);
        pthread_exit(NULL);
    }

    // hit / miss 에 대해서 로그 기록
    if (strcmp(args->type, "Hit") == 0) {
        fprintf(file, "[Hit] ServerPID : %d | %c%c%c/%s - [%s]\n",
                getpid(), args->hashed_url[0], args->hashed_url[1], args->hashed_url[2],
                &args->hashed_url[3], time_buf);
        fprintf(file, "[Hit]%s\n", args->url);
    } else if (strcmp(args->type, "Miss") == 0) {
        fprintf(file, "[Miss] ServerPID : %d | %s - [%s]\n",
                getpid(), args->url, time_buf);
    }
...
void Write_log_in_file( const char* url, const char* hashed_url,const char* type)
{
    ...
    pthread_t tid;
    struct log_args* args = malloc(sizeof(struct log_args));
    args->url = strdup(url);
    args->hashed_url=strdup(hashed_url);
    args->type = strdup(type);

    if(pthread_create(&tid,NULL,log_thread_func,(void*)args)!=0)
    {
    ...
    pthread_join(tid,NULL); // log thraed 종료 대기 
